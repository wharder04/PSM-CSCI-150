...
const REAL_USER_FILTER = { isSystemPlaceholder: { $ne: true } };
...
export const loginUser = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({ email: normalizedEmail, ...REAL_USER_FILTER }).select("+password");
...
export const getCurrentUser = async (req, res) => {
  try {
    if (req.user?.isSystemPlaceholder) {
...
export const forgotPassword = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({ email: normalizedEmail, ...REAL_USER_FILTER });
