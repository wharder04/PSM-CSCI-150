...
export default function TaskBoard() {
  ...
  const fetchBoardData = async (projectId) => {
    ...
    try {
      ...
      const nextMembers = membersRes?.success
        ? membersRes.data
        : membersRes?.data;
  ...
  const tasksWithUiStatus = useMemo(
    () =>
      (tasks || []).map((t) => ({
        ...t,
        uiStatus: normalizeStatusForColumns(t.status),
      })),
    [tasks],
  );

  const filteredTasks = useMemo(() => {
    const { startOfToday, endOfToday, startOfWeek, endOfWeek } =
      getDateRangeAnchors(new Date());
  ...
  const onDragEnd = async (event) => {
    ...
    const overIndex = isOverColumn
      ? toTasks.length
      : toTasks.findIndex((t) => t._id === overId);
  ...
  return (
    <div className="min-h-screen w-full p-2 bg-bg-main">
      <div className="mb-8 max-w-full">
        ...
        <div className="relative flex gap-3 flex-wrap items-center">
          ...
          <button
            ...
            title={
              !activeProjectId ? "Select a project first" : "Create a new task"
            }
          >
            <MdAdd size={18} />
      ...
      {loading ? (
        ...
        <div className="text-text-secondary px-2">
          Create a project first, then come back here.
        </div>
      ) : (
        <DndContext
          sensors={sensors}
