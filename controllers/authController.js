...
const COOKIE_NAME = process.env.COOKIE_NAME || 'psm_token';
...
function sign(sub, remember = true) {
  return jwt.sign({ sub }, process.env.JWT_SECRET, { expiresIn: remember ? EXPIRES_LONG : EXPIRES_SHORT });
}
...
export const registerUser = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ ok: true, data: { user: { _id: user._id, name, email } } });
  } catch (e) { next(e); }
};
...
export const loginUser = async (req, res, next) => {
  try {
    ...
    const token = sign(user._id, remember);
    setAuthCookie(res, token, remember);
    res.json({ ok: true, data: { user: { _id: user._id, name: user.name, email: user.email } } });
  } catch (e) { next(e); }
};
